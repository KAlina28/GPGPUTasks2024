...
bool selectDevice(cl_platform_id &platform, cl_device_id &device) {
    ...
    for (auto platform : platforms) {
        ...
        if (clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, 0, nullptr, &devicesCount) == CL_DEVICE_NOT_FOUND) {
            continue;
        }
        if (devicesCount == 0) {
            continue;
        ...
        OCL_SAFE_CALL(clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, devicesCount, platformDevices.data(), nullptr));
    ...
    for (auto platform : platforms) {
        cl_uint devicesCount = 0;
        if (clGetDeviceIDs(platform, CL_DEVICE_TYPE_CPU, 0, nullptr, &devicesCount) == CL_DEVICE_NOT_FOUND) {
            continue;
        }
        if (devicesCount == 0) {
            continue;
        ...
        std::vector<cl_device_id> platformDevices(devicesCount);
        OCL_SAFE_CALL(clGetDeviceIDs(platform, CL_DEVICE_TYPE_CPU, devicesCount, platformDevices.data(), nullptr));
    ...
    if (!device) {
        throw std::runtime_error("No GPU or CPU found!");
    }
...
int main() {
    ...
    cl_mem as_gpu =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem bs_gpu =
            clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(), &err);
    OCL_SAFE_CALL(err);
    ...
    const char *kernelSourcePtr = kernel_sources.c_str();
    const std::size_t sourceSize = kernel_sources.size();
    cl_program program = clCreateProgramWithSource(context, 1, &kernelSourcePtr, &sourceSize, &err);
    OCL_SAFE_CALL(err);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        std::cout << "GFlops: " << n * 1.0 / t.lapAvg() / 1000000000 << std::endl;
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            cl_event event;
            OCL_SAFE_CALL(
                    clEnqueueReadBuffer(queue, cs_gpu, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(1, &event));
            t.nextLap();
        }
